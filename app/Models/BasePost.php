...
{
    public $table = 'posts';
    public $fillable = ['author_id', 'dynamic_id', 'parent_id', 'category_id', 'category_map', 'type', 'content_type', 'title', 'slug', 'keywords', 'description', 'content', 'features_image', 'views', 'privacy', 'trashed_status'];
    ...
    public function deleteFeatureImage()
    {
        ...
        if ($this->featured_image && file_exists($path = public_path('static/' . $fd . '/' . $this->featured_image))) {
            unlink($path);
            if (file_exists($p = public_path('static/' . $fd . '/90x90/' . $this->featured_image))) {
                unlink($p);
            }
            if (file_exists($p = public_path('static/' . $fd . '/thumbs/' . $this->featured_image))) {
                unlink($p);
            }if (file_exists($p = public_path('static/' . $fd . '/social/' . $this->featured_image))) {
                unlink($p);
            }
        }
    }
